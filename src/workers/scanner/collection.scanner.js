...
export async function scanCollection({ project }) {
    ...
    const requests = await RawRequest.find({ orgId: project.orgId, collectionId: collection._id }).lean();
    ...
    for (let request of requests) {
        ...
        const urlResolvedRequest = substituteUrlVariables(request, environmentVariables);
        const nonUrlResolvedRequest = substituteNonUrlVariables(urlResolvedRequest, environmentVariables);
