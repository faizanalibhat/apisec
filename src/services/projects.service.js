...
import {
  ...
  getTotalResolvedVulnerabilities,
  getTotalRawRequests,
  getTotalTransformedRequests,
  getTotalRules,
  getActiveRulesCount,
...
class ProjectsService {
  ...
  async create(orgId, projectData) {
    try {
      ...
      const activeRules = await Rule.find({ orgId, isActive: true })
        .select("_id")
        .lean();
  ...
  async getProjectDashboard(projectId, orgId, period) {
    try {
      ...
      const remediation =
        totalVulns > 0 ? Math.round((totalResolved / totalVulns) * 100) : 0;
  ...
  async updateRuleSettings(projectId, orgId, ruleData) {
    try {
      ...
      if (!active) {
        ...
        const isLastRule =
          project.includedRuleIds.length === 1 &&
          project.includedRuleIds[0].toString() === ruleId;
        if (isLastRule) {
          throw ApiError.badRequest(
            "A project must have at least one active rule. You cannot remove the last rule.",
          );
        }
      }
      ...
      if (ruleId) {
        ...
        if (validRule !== 1) {
          throw ApiError.badRequest(
            "Rule ID is invalid or does not belong to your organization",
          );
        }
      }
      ...
      const effectiveRules = await this.getEffectiveRules(
        projectId,
        orgId,
        modifiedBy,
      );
  ...
  async updateCollectionsAndRequests(collectionUids, projectId, action) {
    ...
    const rawRequests = await RawRequest.find({
      collectionUid: { $in: collectionUids },
    });
