...
function handleTransformation(body, transformations, mode, language) {
  ...
  for (let transformation of transformations) {
    ...
    allParams.push(...(applyRules(cloneParam, transformation, mode, language) || []));
  }
...
function applyRules(body, rules, mode, language) {
  ...
  if (rules.replace_all_values) {
    modified = replace_all_values(modified, rules.replace_all_values, mode, language);
  }

  console.log("[+] AFTER REPLACE ALL : ", modified);

  if (rules.replace_all_values_one_by_one) {
    allBodies = replace_all_values_one_by_one(modified, rules.replace_all_values_one_by_one, mode, language);
  }
  ...
  if (rules.transformations) {
    if (allBodies.length) {
      ...
      for (let p of allBodies) {
        transformedParams.push(...(handleTransformation(p, rules.transformations, mode, language) || []));
      }

      allBodies.push(...transformedParams);
    }
    else {
      allBodies = handleTransformation(modified, rules.transformations, mode, language);
    }
