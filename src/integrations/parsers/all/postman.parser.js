export class PostmanParser {
  static parseRequests = async (collection, uid) => {
    ...
    return this._parseItems(
      collection.item,
      collection.info?.name || "Postman Collection",
      "",
      uid,
    );
  };
  ...
  static _parseItems = (items, collectionName, folderPath = "", uid) => {
    ...
    for (const item of items) {
      if (item.request) {
        const parsed = this._parseRequest(
          item,
          collectionName,
          folderPath,
          uid,
      ...
      if (item.item && Array.isArray(item.item)) {
        const currentPath = folderPath
          ...
          : item.name;
        requests.push(
          ...this._parseItems(item.item, collectionName, currentPath, uid),
        );
      }
    }
