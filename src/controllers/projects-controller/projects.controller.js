...
class ProjectsController {
  ...
  async createProject(req, res, next) {
    try {
      const { orgId, _id, firstName, lastName, email } =
    ...
    try {
      ...
      if (!validPeriodPattern.test(period)) {
        throw ApiError.badRequest(
          "Invalid period format. Use format like 7d, 30d, 24h, 3m",
        );
      }
      ...
      res.sendApiResponse(
        ApiResponse.success(
          "Project dashboard statistics fetched successfully",
          {
            name: project.name,
            ...dashboardData,
          },
  ...
  async toggleCollectionStatus(req, res, next) {
    try {
      ...
      const project = await this.projectsService.toggleCollectionStatus(
        projectId,
        orgId,
        isCollecting,
      );
  ...
  async addCollection(req, res, next) {
    try {
      ...
      const project = await this.projectsService.addCollection(
        projectId,
        orgId,
        collectionUid,
      );
      res.sendApiResponse(
        ApiResponse.success("Collection added successfully", project),
      );
    } catch (error) {
      next(error);
    }
  ...
  async removeCollection(req, res, next) {
    try {
      ...
      const project = await this.projectsService.removeCollection(
        projectId,
        orgId,
        collectionUid,
      );
      res.sendApiResponse(
        ApiResponse.success("Collection removed successfully", project),
      );
    } catch (error) {
      next(error);
    }
  ...
  async getProjectRules(req, res, next) {
    try {
      ...
      res.sendApiResponse(
        ApiResponse.success("Project rules retrieved successfully", response),
      );
    } catch (error) {
      next(error);
    }
  ...
  async getEffectiveRules(req, res, next) {
    try {
      ...
      const effectiveRules = await this.projectsService.getEffectiveRules(
        projectId,
        orgId,
        userEmail,
      );
      ...
      res.sendApiResponse(
        ApiResponse.success("Effective rules calculated successfully", {
  ...
  async updateProjectRules(req, res, next) {
    try {
      ...
      res.sendApiResponse(
        ApiResponse.success(
          "Project rules updated successfully",
          updatedProject,
        ),
      );
    } catch (error) {
  ...
  async getBrowserRequests(req, res, next) {
    try {
      const { orgId } = req.authenticatedService;
      const { projectId } = req.params;
      const { page = 1, limit = 10, search, sort, method } = req.query;
  ...
  async createBrowserRequest(req, res, next) {
    try {
      ...
      console.log(
        ...
        browserData.request?.request?.method,
        browserData.request?.request?.url?.raw,
      );
      ...
      const rawRequestData = this.transformBrowserRequest(
        browserData,
        project,
        orgId,
        projectId,
      ...
      if (existingRequest) {
        return res.sendApiResponse(
          ApiResponse.success(
            "Request already exists in project (duplicate ignored)",
            {
      ...
      console.log(
        `[+] Created new request: ${newRequest._id} for project ${projectId}`,
      );
      ...
      if (existingTransformations > 0) {
        console.log(
          `[!] Transformations already exist for request ${newRequest._id}, not publishing event`,
        );
        res.sendApiResponse(
          ApiResponse.created(
            "Browser request created successfully (scan already in progress)",
            newRequest,
          ),
        );
        return;
      ...
      console.log(
        `[+] Published request.created event for request ${newRequest._id} in project ${projectId}`,
      );

      res.sendApiResponse(
        ApiResponse.created("Browser request created successfully", newRequest),
      );
    } catch (error) {
      next(error);
    }
  ...
  async bulkCreateBrowserRequests(req, res, next) {
    try {
      ...
      for (const [index, browserData] of requests.entries()) {
        try {
          ...
          if (existingRequest) {
            ...
            console.log(
              `[+] Skipped duplicate request for project ${projectId}`,
            );
          } else {
            // Create new request
            const newRequest = await RawRequest.create(rawRequestData);
            ...
            await mqbroker.publish(
              "apisec",
              "apisec.request.created",
              eventPayload,
            );
            console.log(
              `[+] Published request.created event for project ${projectId}`,
            );
          }
        } catch (error) {
          results.failed.push({
      ...
      const createdCount = results.success.filter(
        (r) => r.status === "created",
      ).length;
  ...
  async getBrowserRequest(req, res, next) {
    try {
      ...
      if (
        !projectIdStrings.includes(projectId) ||
        rawRequest.source !== "browser-extension"
      ) {
      ...
      res.sendApiResponse(
        ApiResponse.success(
          "Browser request retrieved successfully",
          rawRequest,
        ),
      );
    } catch (error) {
  ...
  async updateBrowserRequest(req, res, next) {
    try {
      ...
      const existingRequest = await this.rawRequestService.findOne(
        requestId,
        orgId,
      );

      const projectIdStrings = existingRequest.projectIds.map((id) =>
        id.toString(),
      );

      if (
        !projectIdStrings.includes(projectId) ||
        existingRequest.source !== "browser-extension"
      ) {
        throw ApiError.notFound("Browser request not found in this project");
      }

      // Update the request
      const updated = await this.rawRequestService.update(
        requestId,
        req.body,
        orgId,
      );
      res.sendApiResponse(
        ApiResponse.success("Browser request updated successfully", updated),
      );
    } catch (error) {
      next(error);
    }
  ...
  async deleteBrowserRequest(req, res, next) {
    try {
      ...
      res.sendApiResponse(
        ApiResponse.success("Browser request deleted successfully"),
      );
    } catch (error) {
      next(error);
    }
  ...
  async configureProject(req, res, next) {
    try {
      ...
      const { owner, environment_id, ...configuration } = req.body;
      ...
      // if collection_uids is given - make sure to add projectId to belonging requests
      let collection_uids = configuration.collection_uids || [];

      if (collection_uids.length > 0) {
        await RawRequest.updateMany(
          { collectionUid: { $in: collection_uids } },
  ...
  async uploadAuthScript(req, res, next) {
    try {
      ...
      res.sendApiResponse(
        ApiResponse.success("Auth script uploaded successfully", project),
      );
    } catch (error) {
      next(error);
    }
  ...
  async updateScanSetting(req, res, next) {
    try {
      ...
      res.sendApiResponse(
        ApiResponse.success("Scan settings updated successfully", project),
      );
    } catch (error) {
      next(error);
    }
  ...
  async executeScan(req, res, next) {
    try {
      ...
      await mqbroker.publish("apisec", "apisec.project.scan.launched", {
        project,
        scan,
      });
